...
import type { Ausruestungszuweisung, Personal, Wartungsprotokoll, Ausruestungskatalog, CreateAusruestungszuweisung, CreatePersonal, CreateWartungsprotokoll, CreateAusruestungskatalog } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async createAusruestungszuweisungEntry(fields: CreateAusruestungszuweisung) {
    return callApi('POST', `/apps/${APP_IDS.AUSRUESTUNGSZUWEISUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'ausruestungszuweisung') });
  }
  static async updateAusruestungszuweisungEntry(id: string, fields: Partial<CreateAusruestungszuweisung>) {
    return callApi('PATCH', `/apps/${APP_IDS.AUSRUESTUNGSZUWEISUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'ausruestungszuweisung') });
  }
  ...
  static async createPersonalEntry(fields: CreatePersonal) {
    return callApi('POST', `/apps/${APP_IDS.PERSONAL}/records`, { fields: cleanFieldsForApi(fields as any, 'personal') });
  }
  static async updatePersonalEntry(id: string, fields: Partial<CreatePersonal>) {
    return callApi('PATCH', `/apps/${APP_IDS.PERSONAL}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'personal') });
  }
  ...
  // --- WARTUNGSPROTOKOLL ---
  static async getWartungsprotokoll(): Promise<Wartungsprotokoll[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.WARTUNGSPROTOKOLL}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Wartungsprotokoll[];
    return enrichLookupFields(records, 'wartungsprotokoll');
  }
  static async getWartungsprotokollEntry(id: string): Promise<Wartungsprotokoll | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.WARTUNGSPROTOKOLL}/records/${id}`);
    const record = { record_id: data.id, ...data } as Wartungsprotokoll;
    return enrichLookupFields([record], 'wartungsprotokoll')[0];
  }
  static async createWartungsprotokollEntry(fields: CreateWartungsprotokoll) {
    return callApi('POST', `/apps/${APP_IDS.WARTUNGSPROTOKOLL}/records`, { fields: cleanFieldsForApi(fields as any, 'wartungsprotokoll') });
  }
  static async updateWartungsprotokollEntry(id: string, fields: Partial<CreateWartungsprotokoll>) {
    return callApi('PATCH', `/apps/${APP_IDS.WARTUNGSPROTOKOLL}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'wartungsprotokoll') });
  }
  static async deleteWartungsprotokollEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.WARTUNGSPROTOKOLL}/records/${id}`);
  }

  // --- AUSRUESTUNGSKATALOG ---
  static async getAusruestungskatalog(): Promise<Ausruestungskatalog[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.AUSRUESTUNGSKATALOG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Ausruestungskatalog[];
    return enrichLookupFields(records, 'ausruestungskatalog');
  }
  static async getAusruestungskatalogEntry(id: string): Promise<Ausruestungskatalog | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.AUSRUESTUNGSKATALOG}/records/${id}`);
    const record = { record_id: data.id, ...data } as Ausruestungskatalog;
    return enrichLookupFields([record], 'ausruestungskatalog')[0];
  }
  static async createAusruestungskatalogEntry(fields: CreateAusruestungskatalog) {
    return callApi('POST', `/apps/${APP_IDS.AUSRUESTUNGSKATALOG}/records`, { fields: cleanFieldsForApi(fields as any, 'ausruestungskatalog') });
  }
  static async updateAusruestungskatalogEntry(id: string, fields: Partial<CreateAusruestungskatalog>) {
    return callApi('PATCH', `/apps/${APP_IDS.AUSRUESTUNGSKATALOG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'ausruestungskatalog') });
  }
  static async deleteAusruestungskatalogEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.AUSRUESTUNGSKATALOG}/records/${id}`);
  }
